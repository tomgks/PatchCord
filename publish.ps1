# Builds the release artifact: a single, self-contained VencordAutoUpdater.exe
# that needs no separate .NET install. Output: .\publish\VencordAutoUpdater.exe
#
...
$proj = Join-Path $PSScriptRoot 'src\VencordAutoUpdater.csproj'
...
$exe = Join-Path $OutDir 'VencordAutoUpdater.exe'
if (Test-Path $exe) {
